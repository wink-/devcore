...
class CreateEoPackingListsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('eo_packing_lists', function (Blueprint $table) {
            $table->integer('number')->nullable()->unique('number');
            $table->dateTime('date')->nullable()->index('date');
            $table->string('employee_code', 10)->nullable()->default('Clayton')->index('employee_code');
    ...
    {
        Schema::dropIfExists('eo_packing_lists');
    }
}
