...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.printer.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.printers.update", [$printer->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="printer">{{ trans('cruds.printer.fields.printer') }}</label>
                <input class="form-control {{ $errors->has('printer') ? 'is-invalid' : '' }}" type="text" name="printer" id="printer" value="{{ old('printer', $printer->printer) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $printer->description) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('archive') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="archive" id="archive" value="1" {{ $printer->archive || old('archive', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('revision') ? 'is-invalid' : '' }}" type="number" name="revision" id="revision" value="{{ old('revision', $printer->revision) }}" step="1">
