...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.pdRecord.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.pd-records.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('logger') ? 'is-invalid' : '' }}" name="logger_id" id="logger_id">
                    @foreach($loggers as $id => $logger)
                        <option value="{{ $id }}" {{ old('logger_id') == $id ? 'selected' : '' }}>{{ $logger }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('record_type') ? 'is-invalid' : '' }}" name="record_type_id" id="record_type_id">
                    @foreach($record_types as $id => $record_type)
                        <option value="{{ $id }}" {{ old('record_type_id') == $id ? 'selected' : '' }}>{{ $record_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('command') ? 'is-invalid' : '' }}" type="text" name="command" id="command" value="{{ old('command', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('multiplier') ? 'is-invalid' : '' }}" type="number" name="multiplier" id="multiplier" value="{{ old('multiplier', '') }}" step="0.0001">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('graph_y_lower') ? 'is-invalid' : '' }}" type="number" name="graph_y_lower" id="graph_y_lower" value="{{ old('graph_y_lower', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('graph_y_upper') ? 'is-invalid' : '' }}" type="number" name="graph_y_upper" id="graph_y_upper" value="{{ old('graph_y_upper', '') }}" step="1">
