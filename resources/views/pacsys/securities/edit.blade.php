...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.security.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("pacsys.securities.update", [$security->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="emp_code">{{ trans('cruds.security.fields.emp_code') }}</label>
                <input class="form-control {{ $errors->has('emp_code') ? 'is-invalid' : '' }}" type="text" name="emp_code" id="emp_code" value="{{ old('emp_code', $security->emp_code) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('scope') ? 'is-invalid' : '' }}" type="text" name="scope" id="scope" value="{{ old('scope', $security->scope) }}">
                @if($errors->has('scope'))
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('time_stamp') ? 'is-invalid' : '' }}" type="text" name="time_stamp" id="time_stamp" value="{{ old('time_stamp', $security->time_stamp) }}">
