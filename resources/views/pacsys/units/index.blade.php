...
@can('p_unit_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('pacsys.p-units.create') }}">
                ...
                <tbody>
                    @foreach($pUnits as $key => $pUnit)
                        <tr data-entry-id="{{ $pUnit->id }}">
                            ...
                            <td>
                                @can('p_unit_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('pacsys.p-units.show', $pUnit->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('p_unit_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('pacsys.p-units.edit', $pUnit->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('p_unit_delete')
                                    <form action="{{ route('pacsys.p-units.destroy', $pUnit->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('p_unit_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('pacsys.p-units.massDestroy') }}",
