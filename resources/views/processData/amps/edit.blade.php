...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.amp.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.amps.update", [$amp->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('record') ? 'is-invalid' : '' }}" name="record_id" id="record_id">
                    @foreach($records as $id => $record)
                        <option value="{{ $id }}" {{ (old('record_id') ? old('record_id') : $amp->record->id ?? '') == $id ? 'selected' : '' }}>{{ $record }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amp') ? 'is-invalid' : '' }}" type="number" name="amp" id="amp" value="{{ old('amp', $amp->amp) }}" step="1">
                @if($errors->has('amp'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amp_minute') ? 'is-invalid' : '' }}" type="number" name="amp_minute" id="amp_minute" value="{{ old('amp_minute', $amp->amp_minute) }}" step="1">
