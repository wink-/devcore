...
@can('pd_unit_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('processdata.pd-units.create') }}">
                ...
                <tbody>
                    @foreach($pdUnits as $key => $pdUnit)
                        <tr data-entry-id="{{ $pdUnit->id }}">
                            ...
                            <td>
                                @can('pd_unit_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('processdata.pd-units.show', $pdUnit->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('pd_unit_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('processdata.pd-units.edit', $pdUnit->id) }}">
                                        Edit
                                    </a>
                                @endcan

                                @can('pd_unit_delete')
                                    <form action="{{ route('processdata.pd-units.destroy', $pdUnit->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('pd_unit_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('processdata.pd-units.massDestroy') }}",
