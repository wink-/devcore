...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.logger.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.loggers.update", [$logger->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="active" value="0">
                    <input class="form-check-input" type="checkbox" name="active" id="active" value="1" {{ $logger->active || old('active', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $logger->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $logger->description) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('logger_type') ? 'is-invalid' : '' }}" name="logger_type_id" id="logger_type_id">
                    @foreach($logger_types as $id => $logger_type)
                        <option value="{{ $id }}" {{ (old('logger_type_id') ? old('logger_type_id') : $logger->logger_type->id ?? '') == $id ? 'selected' : '' }}>{{ $logger_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ip_address') ? 'is-invalid' : '' }}" type="text" name="ip_address" id="ip_address" value="{{ old('ip_address', $logger->ip_address) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('port') ? 'is-invalid' : '' }}" type="number" name="port" id="port" value="{{ old('port', $logger->port) }}" step="1">
                @if($errors->has('port'))
