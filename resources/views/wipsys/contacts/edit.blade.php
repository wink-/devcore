...
<div class="card">
    <div class="card-header">
        Edit {{ trans('cruds.contact.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("wipsys.contacts.update", [$contact->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('customer') ? 'is-invalid' : '' }}" name="customer_id" id="customer_id">
                    @foreach($customers as $id => $customer)
                        <option value="{{ $id }}" {{ (old('customer_id') ? old('customer_id') : $contact->customer->id ?? '') == $id ? 'selected' : '' }}>{{ $customer }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quote_emails') ? 'is-invalid' : '' }}" type="text" name="quote_emails" id="quote_emails" value="{{ old('quote_emails', $contact->quote_emails) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('email_quote_print') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="email_quote_print" id="email_quote_print" value="1" {{ $contact->email_quote_print || old('email_quote_print', 0) === 1 ? 'checked' : '' }}>
