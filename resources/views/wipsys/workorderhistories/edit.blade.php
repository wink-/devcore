...
<div class="card">
    <div class="card-header">
        Edit {{ trans('cruds.workorderhistory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("wipsys.workorderhistories.update", [$workorderhistory->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('part') ? 'is-invalid' : '' }}" name="part_id" id="part_id" required>
                    @foreach($parts as $id => $part)
                        <option value="{{ $id }}" {{ (old('part_id') ? old('part_id') : $workorderhistory->part->id ?? '') == $id ? 'selected' : '' }}>{{ $part }}</option>
                    @endforeach
                </select>
                @if($errors->has('part'))
