...
@can('priority_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('wipsys.priorities.create') }}">
                ...
                <tbody>
                    @foreach($priorities as $key => $priority)
                        <tr data-entry-id="{{ $priority->id }}">
                            ...
                            <td>
                                <span style="display:none">{{ $priority->archive ?? '' }}</span>
                                <input type="checkbox" disabled="disabled" {{ $priority->archive ? 'checked' : '' }}>
                            </td>
                            <td>
                                @can('priority_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('wipsys.priorities.show', $priority->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('priority_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('wipsys.priorities.edit', $priority->id) }}">
                                        Edit
                                    </a>
                                @endcan

                                @can('priority_delete')
                                    <form action="{{ route('wipsys.priorities.destroy', $priority->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('priority_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('wipsys.priorities.massDestroy') }}",
