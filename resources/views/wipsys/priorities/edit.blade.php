...
<div class="card">
    <div class="card-header">
        Edit {{ trans('cruds.priority.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("wipsys.priorities.update", [$priority->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="code">{{ trans('cruds.priority.fields.code') }}</label>
                <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="text" name="code" id="code" value="{{ old('code', $priority->code) }}">
                @if($errors->has('code'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $priority->description) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('image_key') ? 'is-invalid' : '' }}" type="text" name="image_key" id="image_key" value="{{ old('image_key', $priority->image_key) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('archive') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="archive" id="archive" value="1" {{ $priority->archive || old('archive', 0) === 1 ? 'checked' : '' }}>
