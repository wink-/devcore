...
<div class="card">
    <div class="card-header">
        Edit {{ trans('cruds.workorder.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("wipsys.workorders.update", [$workorder->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('part') ? 'is-invalid' : '' }}" name="part_id" id="part_id" required>
                    @foreach($parts as $id => $part)
                        <option value="{{ $id }}" {{ (old('part_id') ? old('part_id') : $workorder->part->id ?? '') == $id ? 'selected' : '' }}>{{ $part }}</option>
                    @endforeach
                </select>
                @if($errors->has('part'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('printed') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="printed" id="printed" value="1" {{ $workorder->printed || old('printed', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="printed">{{ trans('cruds.workorder.fields.printed') }}</label>
                </div>
                @if($errors->has('printed'))
                    <span class="text-danger">{{ $errors->first('printed') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.workorder.fields.printed_helper') }}</span>
            </div>
            <div class="form-group">
