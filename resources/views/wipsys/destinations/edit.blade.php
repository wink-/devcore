...
<div class="card">
    <div class="card-header">
        Edit {{ trans('cruds.destination.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("wipsys.destinations.update", [$destination->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('customer_code') ? 'is-invalid' : '' }}" name="customer_code_id" id="customer_code_id">
                    @foreach($customer_codes as $id => $customer_code)
                        <option value="{{ $id }}" {{ (old('customer_code_id') ? old('customer_code_id') : $destination->customer_code->id ?? '') == $id ? 'selected' : '' }}>{{ $customer_code }}</option>
                    @endforeach
                </select>
